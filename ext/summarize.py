...
def summarize_html(html, maxwords = 25):
    ...
    if not do_not_add_dots:
        result = alist[:i] + ['... <a href="#" class="continue">weiterlesen</a>.'] + ['</%s>'%x[x.find('<')+1:_find_ws_or_gt(x,1)] for x in tagstack]
    elif was_table_row:
...
def cb_prepare(request):
    ...
    for i, entry in enumerate(data['entry_list']):
        ...
        if t == 'page':
            data['entry_list'][i]['body'] = summarize_html(entry['body'], 100)

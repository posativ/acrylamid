...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        keywords = {'skip': self.BLACK, 'create': self.GREEN}
        for item in keywords:
            if record.msg.find(item) == 0:
                record.msg = record.msg.replace(item, ' '*6 + \
                                keywords[item] % item)
        
        # record.msg = record.msg.replace('skip', ' '*6 + self.BLACK % 'skip')
        # record.msg = record.msg.replace('create', ' '*6 + self.GREEN % 'create')
